...
const roomInfoFormWrap = document.getElementById("roomInfoFormWrap");
...
if (saveRoomInfoBtn) {
    saveRoomInfoBtn.addEventListener("click", function () {
        const rlRoomName = roomNameInput.value.trim();
        const rlRoomPassword = roomPasswordInput.value.trim();

        if (!rlRoomName || !rlRoomPassword) {
            showToast("Enter both a Private Match name and password.", "error");
            return;
        }

        saveRoomInfoBtn.disabled = true;
        saveRoomInfoBtn.textContent = "SAVING...";

        apiFetch("/api/matches/" + currentMatchId + "/room-info", {
            method: "POST",
            body: JSON.stringify({ rlRoomName: rlRoomName, rlRoomPassword: rlRoomPassword })
        })
            .then(function (data) {
                saveRoomInfoBtn.disabled = false;
                saveRoomInfoBtn.textContent = "SAVE ROOM INFO";

                if (!data.success) {
                    showToast(data.message || "Could not save room info.", "error");
                    return;
                }
            ...
            .catch(function (error) {
                console.log("SAVE ROOM INFO ERROR:", error);
                saveRoomInfoBtn.disabled = false;
                saveRoomInfoBtn.textContent = "SAVE ROOM INFO";
                showToast("Could not save room info. Make sure your backend server is running.", "error");
            });
    });
}
...
// Only the host (creator) can set the Private Match name/password, and only
// once - the private match is usually made in Rocket League after an
// opponent is already known, so this is a separate step from joining, not
// part of match creation (see POST /api/matches/:id/room-info).
function updateRoomInfoSection(match, isCreator) {
    if (match.rlRoomName && match.rlRoomPassword) {
        roomInfoCard.classList.remove("hidden");
        roomInfoFormWrap.classList.add("hidden");

        roomNameDisplay.textContent = match.rlRoomName;
        roomPasswordDisplay.textContent = match.rlRoomPassword;
        return;
    }
    ...
    if (isCreator) {
        roomInfoFormWrap.classList.remove("hidden");
    } else {
        roomInfoFormWrap.classList.add("hidden");
    }
}
...
function updateRoomState(match) {
    ...
    // Rocket League IDs + instructions immediately, and both hit Ready before
    ...
    if (match.status === "Match ready") {
        ...
        updateRoomInfoSection(match, isCreator);
