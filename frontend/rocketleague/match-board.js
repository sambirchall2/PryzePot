...
function attachButtonListeners() {
    // Rocket League ID is always confirmed at connect-rocketleague.html before
    // joining (same pattern as creation, and same as Madden's EA name -
    // never skipped even if one's already saved). That page resumes the
    // join itself once the ID is confirmed (see connect-rocketleague.js's
    // pendingJoinMatchId handling).
...
function loadMatches() {
    apiFetch("/api/matches?game=" + encodeURIComponent("Rocket League"))
        .then(function (data) {
            if (!data.success) {
                matchesContainer.innerHTML = `
                    <div class="empty-state">
